...
nhn.husky.SE2M_AttachQuickPhoto = jindo.$Class({		
	...
	makePopupURL : function(){
		var sPopupUrl = "/SmartEditorTest/se/sample/photo_uploader/photo_uploader.html";
	...
	$ON_SET_PHOTO : function(aPhotoData){
		...
		try{
			...
			for(var i = 0; i <aPhotoData.length; i++){				
				...
				console.log(myPhotoData.loadCnt + ":" + aPhotoInfo.sOriginalImageURL);
				sContents += this._getPhotoTag(aPhotoInfo);
				
				if(myPhotoData.loadCnt == 0){
					//썸네일 생성 로직
	...
	_getPhotoTag : function(htPhotoInfo){
		...
		var sTag = '<img class="leeImg" src="{=sOriginalImageURL}" title="{=sName}" >';
